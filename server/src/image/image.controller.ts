...
export class ImageController {
  ...
  @Get('status')
  @ApiOperation({
    summary: 'Check if AI image generation features are available',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 200,
    description: 'Image(s) successfully generated',
  ...
  async generateImage(
    @Body() request: ImageGenerateRequestDto,
  ): Promise<ImageResponseDto> {
    try {
      this.logger.debug(
        `Received image generation request for prompt: "${request.prompt}"`,
      );
      ...
      this.logger.error(
        `Error in image generation controller: ${err.message}`,
        err.stack,
      );
