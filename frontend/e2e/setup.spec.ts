...
test.describe('Setup Mode', () => {
  ...
  test.describe('Initial Setup Flow', () => {
    ...
    test('shows both offline and server options when both modes are available', async ({
      ...
      await expect(page.getByTestId('offline-mode-button')).toBeVisible();
      await expect(
        page.locator('text=Use Inkweld locally without an internet connection')
      ).toBeVisible();

      // Should show server mode option
      await expect(page.getByTestId('server-mode-button')).toBeVisible();
      await expect(
  ...
  test.describe('Offline Mode Setup', () => {
    test('can configure offline mode successfully', async ({
      ...
      // Fill in the form
      await page
        .getByTestId('offline-username-input')
        .fill('testuser');
      await page
        .getByTestId('offline-displayname-input')
        .fill('Test User');
    ...
    test('can go back from offline setup to mode selection', async ({
      ...
      await expect(
        page.getByTestId('offline-back-button')
      ).toBeVisible();

      // Click back button
      await page.getByTestId('offline-back-button').click();
  ...
  test.describe('Server Mode Setup', () => {
    test('can configure server mode successfully', async ({
      ...
      await page
        .getByTestId('server-url-input').fill('https://test-server.com');
  ...
  test.describe('UI and Accessibility', () => {
    test('setup form has proper accessibility attributes', async ({
      ...
      const usernameInput = page.getByTestId('offline-username-input');
      const displayNameInput = page.getByTestId('offline-displayname-input');
      ...
      await expect(
        page.getByTestId('start-offline-button')
      ).toBeVisible();
