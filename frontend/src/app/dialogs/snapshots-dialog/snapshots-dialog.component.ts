...
/**
 * Data passed to the SnapshotsDialog
 */
export interface SnapshotsDialogData {
  /** Document ID to show snapshots for */
  documentId: string;
  /** Optional current word count for display */
  currentWordCount?: number;
}

/**
 * Dialog component for managing document snapshots.
 * Allows users to view, create, restore, and delete snapshots.
 */
@Component({
...
export class SnapshotsDialogComponent implements OnInit {
  ...
  async createSnapshot(): Promise<void> {
    ...
    >(CreateSnapshotDialogComponent, {
      width: '500px',
      data: {
        wordCount: this.data.currentWordCount,
      },
    });
  ...
  async restoreSnapshot(snapshot: UnifiedSnapshot): Promise<void> {
    ...
    >(RestoreSnapshotDialogComponent, {
      ...
      data: {
        snapshot,
        currentWordCount: this.data.currentWordCount,
      },
    });
