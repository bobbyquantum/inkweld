...
export class UserAvatarComponent implements OnInit, OnChanges, OnDestroy {
  ...
  protected readonly avatarUrl = signal<SafeUrl | undefined>(undefined);
  protected readonly fallbackAvatarUrl = signal<string | undefined>(undefined);
  ...
  private generateFallbackAvatar(): void {
    ...
    this.fallbackAvatarUrl.set(
      generateFracticonDataURL(this.username, {
        size,
        circular: true,
      })
    );
