...
/**
 * Local implementation of the element sync provider.
 *
 * Uses LocalProjectElementsService (Yjs + IndexedDB locally)
 ...
 *
 * This provider:
 * - Always reports DocumentSyncState.Local
 * - Persists all data to IndexedDB via LocalProjectElementsService
 * - Does not require network connectivity
 */
@Injectable({
  providedIn: 'root',
})
export class LocalElementSyncProvider implements IElementSyncProvider {
  private readonly logger = inject(LoggerService);
  private readonly localService = inject(LocalProjectElementsService);
  ...
  async connect(config: SyncConnectionConfig): Promise<SyncConnectionResult> {
    ...
    try {
      ...
      await this.localService.loadElements(username, slug);
  ...
  disconnect(): void {
    ...
    if (this.currentUsername && this.currentSlug) {
      void this.localService.closeConnection(
