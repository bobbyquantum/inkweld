 ...
 * This enables local snapshot creation and restoration.
 *
 ...
 * ```typescript
 * // Create a snapshot
 * const snapshot = await localSnapshot.createSnapshot('alice/my-novel', 'doc-123', {
 ...
 * const snapshots = await localSnapshot.listSnapshots('alice/my-novel', 'doc-123');
 *
 * // Get full snapshot for restore
 * const full = await localSnapshot.getSnapshot('alice/my-novel', 'doc-123', snapshotId);
 *
 * // Delete a snapshot
 * await localSnapshot.deleteSnapshot('alice/my-novel', 'doc-123', snapshotId);
 * ```
 */
@Injectable({
  providedIn: 'root',
...
export class LocalSnapshotService {
  ...
  async deleteSnapshot(
    ...
    this.logger.debug('LocalSnapshot', `Deleted snapshot ${snapshotId}`);
    void this.updatePendingSync();
  }
  ...
  async deleteSnapshotById(id: string): Promise<void> {
    const db = await this.ensureDb();
    await this.storageService.delete(db, STORE_NAME, id);
    this.logger.debug('LocalSnapshot', `Deleted snapshot ${id}`);
    void this.updatePendingSync();
  }
