...
/**
 * Service to handle migration of local data to a server
 *
 * Migration process:
 * 1. Project metadata - Sent to server via API
 * 2. Elements - Automatically synced via Yjs when WebSocket connects
 *    (Both local and online modes use Yjs + IndexedDB, so sync is seamless)
 * 3. Documents - Automatically synced via Yjs when WebSocket connects
 *    (Both local and online modes use Yjs + IndexedDB, so sync is seamless)
 *
 * The Yjs CRDT architecture ensures that local data in IndexedDB automatically
...
export class MigrationService {
  ...
  hasLocalProjects(): boolean {
    return this.localProjectService.projects().length > 0;
  }

  /**
   * Get count of local projects
   */
  getLocalProjectsCount(): number {
    return this.localProjectService.projects().length;
  }

  /**
   * Migrate all local projects to the server
   *
   * @param serverUrl - The server URL to migrate to
   * @returns Promise that resolves when migration is complete
   */
  async migrateToServer(serverUrl: string): Promise<void> {
    const localProjects = this.localProjectService.projects();
    ...
    this.logger.info(
      'MigrationService',
      `Starting migration of ${localProjects.length} projects to ${serverUrl}`
    );
  ...
  private async migrateProject(project: Project): Promise<void> {
    ...
    try {
      ...
      // The local elements are already stored in IndexedDB using Yjs.
   ...
   * WARNING: This will delete all local projects, elements, and user data
   */
  cleanupLocalData(): void {
    ...
    // 1. Documents might still be useful for local editing
