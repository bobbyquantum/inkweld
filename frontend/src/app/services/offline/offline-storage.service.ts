 ...
 * await offlineStorage.saveMedia('alice/my-novel', 'cover', coverBlob);
 *
 * // Get a blob URL for display
 * const url = await offlineStorage.getMediaUrl('alice/my-novel', 'cover');
 *
 * // Save an inline image
 * await offlineStorage.saveMedia('alice/my-novel', `img-${uuid}`, imageBlob);
 *
 * // List all images in a project
 * const images = await offlineStorage.listMedia('alice/my-novel');
 *
 * // Cleanup when closing project
 * offlineStorage.revokeProjectUrls('alice/my-novel');
 * ```
 */
@Injectable({
  providedIn: 'root',
