...
describe('MigrationService', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        { provide: OfflineProjectElementsService, useValue: {} },
      ],
    });
  ...
  describe('initial state', () => {
    ...
    it('should detect offline projects when they exist', () => {
      ...
      expect(service.getOfflineProjectsCount()).toBe(2);
    });
  });
  ...
  describe('migrateToServer', () => {
    it('should do nothing when no offline projects exist', async () => {
      ...
      expect(loggerMock.info).toHaveBeenCalledWith(
        'MigrationService',
        'No offline projects to migrate'
      );
    });
    ...
    it('should handle 409 conflict (project already exists) gracefully', async () => {
      offlineProjectService.projects.set([mockProjects[0]]);
    ...
    it('should handle projects with undefined description', async () => {
      ...
      offlineProjectService.projects.set([projectWithoutDescription]);
      projectsServiceMock.createProject.mockReturnValue(
  ...
  describe('cleanupOfflineData', () => {
    it('should delete all offline projects', () => {
      offlineProjectService.projects.set(mockProjects);

      service.cleanupOfflineData();

      expect(offlineProjectService.deleteProject).toHaveBeenCalledTimes(2);
    ...
    it('should clear offline localStorage items', () => {
      localStorage.setItem('inkweld-offline-elements', 'test-elements');
      localStorage.setItem('inkweld-offline-user', 'test-user');
      localStorage.setItem('auth_token', 'should-remain');

      service.cleanupOfflineData();

      expect(localStorage.getItem('inkweld-offline-elements')).toBeNull();
      expect(localStorage.getItem('inkweld-offline-user')).toBeNull();
    ...
    it('should handle empty projects list', () => {
      offlineProjectService.projects.set([]);

      service.cleanupOfflineData();

      expect(offlineProjectService.deleteProject).not.toHaveBeenCalled();
      expect(loggerMock.info).toHaveBeenCalledWith(
        'MigrationService',
        'Offline data cleanup completed'
      );
