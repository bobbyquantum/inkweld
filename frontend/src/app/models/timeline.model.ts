...
export interface TimelineEvent {
  ...
  color?: string;
  /**
   * Origin of the event. `'manual'` (default, user-authored) or `'auto'`
   * (generated by auto-build from element date fields). Auto-built events
   * are replaced on the next auto-build run; manual events are preserved.
   */
  source?: 'auto' | 'manual';
  /**
   * Schema field key that sourced this auto-built event (e.g. `'birthDate'`
   * or `'appearance.born'`). Present only on `source: 'auto'` events.
   * Used as the idempotency key alongside `linkedElementId` so re-runs
   * update existing events in place rather than duplicating.
   */
