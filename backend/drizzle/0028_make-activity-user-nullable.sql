...
--
-- No PRAGMA foreign_keys toggle is needed here: nothing references
-- activity_events via an inbound foreign key, so the rebuild cannot trip a
-- constraint. (The Drizzle Bun-SQLite migrator also wraps migrations in a
-- transaction, where toggling PRAGMA foreign_keys is a no-op anyway.)
