...
-- project_collaborators(user_id, status, invited_at): collaborator-side
--   lookups (pending invitations sorted by invited_at, projects shared with
--   me, OAuth grants by granting user). All existing indexes led with
--   project_id or mcp_session_id.
-- document_snapshots(project_id, document_id, created_at): snapshot listings,
--   newest first; rows carry the full XML payload so scans are expensive per
--   row. The trailing sort column avoids a temporary B-tree for ORDER BY.
-- project_tombstones(user_id): the PK is (slug, user_id) so per-user lookups
--   could not use it.
-- published_files(project_id, created_at) and (share_token): per-project
--   listings (newest first) and anonymous share-link resolution.
--
