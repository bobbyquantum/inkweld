...
public class FileContentController {
    ...
    @Operation(summary = "Get file meta", description = "Gets file meta for a file owned by the currently authenticated user.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "File meta returned successfully",
                    content = @Content(
                            mediaType = MediaType.APPLICATION_JSON_VALUE,
    ...
    @Parameter(name = "download", description = "Set to true to download the file, false to display inline", required = false)
    @ApiResponses(value = {
                ...
                .map(file -> {
                    ...
                    try {
                        if (rangeHeader == null) {
                            ...
                            headers.add(HttpHeaders.CONTENT_RANGE, "bytes " + start + "-" + end + "/" + file.getContentLength());
    ...
    @Operation(summary = "Update file metadata", description = "Updates metadata for a file owned by the currently authenticated user.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "File metadata updated successfully",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE, schema = @Schema(implementation = FileDto.class))),
    ...
    public ResponseEntity<?> patchFile(
        ...
        return fileService.patchFile(fileId, patchDto)
    ...
    @Operation(summary = "Delete file", description = "Deletes a file owned by the currently authenticated user.")
    @ApiResponses(value = {
    ...
    @Operation(
            ...
            description = "Searches for files owned by the currently authenticated user based on the provided criteria."
    )
    @ApiResponses(value = {
            @ApiResponse(
                    responseCode = "200",
    ...
    public ResponseEntity<Page<FileDto>> searchFiles(
            ...
            @Parameter(description = "Pageable information for the search results")
            Pageable pageable
    ) {
        ...
        PageImpl<FileDto> page = new PageImpl<>(fileDtos, files.getPageable(), files.getTotalElements());
