...
public class JpaTokenRepository implements PersistentTokenRepository {
    ...
    public void updateToken(String series, String tokenValue, Date lastUsed) {
        PersistentToken token = entityManager.createQuery(
    ...
    public PersistentRememberMeToken getTokenForSeries(String seriesId) {
        ...
        if (token != null) {
            return new PersistentRememberMeToken(token.getUsername(), token.getSeries(),
                    token.getTokenValue(), convertToDate(token.getLastUsed()));
        } else {
            return null;
        }
    }
    ...
    public void removeUserTokens(String username) {
        entityManager.createQuery("DELETE FROM PersistentToken t WHERE t.username = :username")
