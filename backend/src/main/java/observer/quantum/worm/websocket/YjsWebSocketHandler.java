...
public class YjsWebSocketHandler extends AbstractWebSocketHandler {
    ...
    public void afterConnectionEstablished(@NonNull WebSocketSession session) {
        ...
        log.info("WebSocket connection established - Session ID: {}, Document ID: {}, URI: {}", 
            session.getId(), documentId, session.getUri());
        
        documentSessions.computeIfAbsent(documentId, k -> new CopyOnWriteArrayList<>()).add(session);
        log.info("Added session to document {} - Total sessions for document: {}", 
    ...
    protected void handleTextMessage(@NonNull WebSocketSession session, @NonNull TextMessage message) throws IOException {
        String documentId = getDocumentIdFromSession(session);
        log.debug("Received text message from session {} for document {} - Message length: {}", 
            session.getId(), documentId, message.getPayloadLength());
    ...
    protected void handleBinaryMessage(@NonNull WebSocketSession session, @NonNull BinaryMessage message) throws IOException {
        String documentId = getDocumentIdFromSession(session);
        log.debug("Received binary message from session {} for document {} - Message length: {}", 
            session.getId(), documentId, message.getPayloadLength());

        broadcastMessage(session, message, documentId);
    }

    private void broadcastMessage(WebSocketSession sender, org.springframework.web.socket.WebSocketMessage<?> message, String documentId) {
            ...
            for (WebSocketSession clientSession : sessions) {
                if (clientSession.isOpen() && !clientSession.getId().equals(sender.getId())) {
                    try {
                        ...
                        log.error("Failed to send message to session {} - Error: {}", 
                            clientSession.getId(), e.getMessage(), e);
                    }
                }
            }
    ...
    public void handleTransportError(@NonNull WebSocketSession session, @NonNull Throwable exception) {
        String documentId = getDocumentIdFromSession(session);
        log.error("Transport error for session {} in document {} - Error: {}", 
            session.getId(), documentId, exception.getMessage(), exception);
    }

    @Override
    public void afterConnectionClosed(@NonNull WebSocketSession session, @NonNull CloseStatus status) {
        String documentId = getDocumentIdFromSession(session);
        log.info("WebSocket connection closed - Session ID: {}, Document ID: {}, Status: {}", 
        ...
        if (sessions != null) {
            ...
            if (sessions.isEmpty()) {
                ...
                log.info("Removed session from document {} - Remaining sessions: {}", 
    ...
    private String getDocumentIdFromSession(@NonNull WebSocketSession session) {
        try {
            ...
            log.error("Error extracting documentId from session - URI: {} - Error: {}", 
                session.getUri(), e.getMessage(), e);
        }
        log.warn("Using default documentId for session {} - URI: {}", 
