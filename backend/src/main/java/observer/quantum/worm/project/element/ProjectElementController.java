...
public class ProjectElementController {
    ...
    @Operation(summary = "Get all elements for a project", 
               description = "Retrieves all elements belonging to the specified project in their hierarchical order")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Successfully retrieved elements",
                    content = @Content(mediaType = MediaType.APPLICATION_JSON_VALUE,
                    array = @ArraySchema(schema = @Schema(implementation = ProjectElementDto.class)))),
    ...
    @Operation(summary = "Create a new element", 
               description = "Creates a new element in the specified project. Position will be automatically set if not provided.")
    @ApiResponses(value = {
    ...
    @Operation(summary = "Update an element", 
               description = "Updates an existing element in the specified project. Can update name, type, parent, and position.")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Element updated successfully",
                    content = @Content(schema = @Schema(implementation = ProjectElementDto.class))),
        @ApiResponse(responseCode = "404", description = "Project or element not found",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
    @PutMapping(path = "/{elementId}", 
    ...
    public ResponseEntity<ProjectElementDto> updateElement(
            ...
            @RequestHeader(name = "X-XSRF-TOKEN") String csrfToken,
            @Valid @RequestBody ProjectElementDto elementDto) {
        return ResponseEntity.ok(elementService.updateElement(username, projectSlug, elementId, elementDto));
    }
    ...
    @Operation(summary = "Update element position", 
